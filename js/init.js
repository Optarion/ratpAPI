			...
			if (xmlhttp.readyState == XMLHttpRequest.DONE ) {
				if (xmlhttp.status == 200) {
					var data = JSON.parse(xmlhttp.response),
						...
						lineType = capitalizeWords(responseInfo.type),
...
console.log('response', response)
					...
					document.getElementById("breadcrumb").innerHTML = station + ' / ' + lineType + ' ' + lineId;

					document.getElementById("nextSchedule").innerHTML = schedulesArray[0]["message"];
					document.getElementById("otherSchedule").innerHTML = schedulesArray[1]["message"];
				}
				else {
